...
$(document.body).ready(function() {
	...
	$('.bumpbox').click(function(e){
		...
		if($(this).attr('href') != null){
			//hr = $(this)[0].pathname;	// removed hr = $(this).attr('href'); because it included hash
			hr = $(this).attr('href');
		}
...
//		var el = $('<div style="width:1px;height:1px;position:absolute;padding:4px;background:#000 url(' + path + 'ajax-loader.gif) no-repeat center center;left:' + middleH + 'px;top:' + (middleV+s) + 'px;display:block;z-index:100000;-moz-border-radius:10px;-webkit-border-radius:10px;border-radius:10px;border:2px solid #777;box-shadow: 0 0 16px #4c4c4c;-moz-box-shadow: 0 0 16px #4c4c4c;-webkit-box-shadow: 0 0 16px #4c4c4c;"></div>');
		var el = $('<div id="bumpbox"></div>');
		el.css('left',middleH + 'px');
		el.css('top',(middleV+s) + 'px');

		var bg = $('<div id="bumpbox_modal" style="background:#000;width:100%;height:100%;opacity:0.9;position:absolute;top:' + s + 'px;left:0;z-index:99999;"></div>');
//		var bg = $('<div id="bumpbox_modal"></div>');
//		el.css('top',s + 'px');

	//	var cl = $('<img src="' + path + 'closed.png" style="cursor:pointer;width:24px;height:24px;position:absolute;top:-16px;right:-16px;z-index:100000;" />');
	var cl = $('<div id="bumpbox_close">X</div>');
...
//						var div = $('<div id="flashcontent" style="padding:20px;width:' + maxwInnerDiv + 'px;height:' + maxhInnerDiv + 'px;"></div>');
						var div = $('<div id="flashcontent" class="bumpbox_image"></div>');
						div.width = maxwInnerDiv;
						div.height = maxhInnerDiv;
						el.append(div);
...
var attributes = {};
swfobject.embedSWF(content, 'flashcontent', maxwInnerDiv, maxhInnerDiv, '9.0.115', path + 'expressInstall.swf', flashvars, params, attributes);
...
*/
						...
						// video
//						var div = $('<div id="flashcontent" style="padding:20px;width:' + maxwInnerDiv + 'px;height:' + maxhInnerDiv + 'px;"></div>');
						var div = $('<div id="flashcontent" class="bumpbox_movie"></div>');
						div.width = maxwInnerDiv;
						div.height = maxhInnerDiv;
						el.append(div);

						var f = flowplayer('flashcontent', {
							src: path + 'flowplayer-3.2.7.swf',
//							buffering: true,
//							autoplay: true,
//							clip: content,
							wmode: 'transparent',	// required to keep it behind modal dialog boxes
...
//						var div = $('<div id="flashcontent" style="padding:20px;width:' + maxwInnerDiv + 'px;height:' + (maxh-50) + 'px;"></div>');
						var div = $('<div id="flashcontent" class="bumpbox_mp3"></div>');
						div.width = maxwInnerDiv;
						div.height = maxh-50;
						el.append(div);
...
//								'coverart.jpg',
							...
							plugins: {
// this does not actually seem to be needed, it will use the audio plugin if .mp3 encountered
audio: {
	url: path + 'flowplayer.audio-3.2.2.swf'
},
								content: {
									url: path + 'flowplayer.content-3.2.0.swf',
...
//						var div = $('<div style="overflow:hidden;padding:20px;width:' + maxwInnerDiv + 'px;height:' + maxhInnerDiv + 'px;"></div>');
					...
					// add title to top
					if(title != ''){
						var t = $('<div id="bumpbox_title">' + title + '</div>');
						el.append(t);
					}

					// add URL: to bottom as long as it is not inline content
					if(content.indexOf('^') != 0){
						var x1 = $('<a href="' + hr + '" id="bumpbox_url">URL: ' + hr + '</a>');
						el.append(x1);
					}
				}
			}
...
//			var nx = $('<a style="cursor:pointer;width:30px;height:20px;background:#000 url(' + path + 'next.jpg) no-repeat center center;position:absolute;right:10px;bottom:-10px;display:block;"></a>');
			var nx = $('<a id="bumpbox_right">&raquo;</a>');
...
//			var nx2 = $('<a style="cursor:pointer;width:30px;height:20px;background:#000 url(' + path + 'prev.jpg) no-repeat center center;position:absolute;right:50px;bottom:-10px;display:block;"></a>');
			var nx2 = $('<a id="bumpbox_left">&laquo;</a>');
